...
const EditListing = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await updateDoc(doc(db, "listings", params.Id), submitData);

      toast.success("Edit Successful");
      navigate(`/category/${submitData.type}/${params.Id}`);
    } catch (error) {
      console.error(
    ...
    const fetchData = async () => {
      try {
        ...
        if (listing?.userRef !== userId) {
          toast.error("Insufficient Permission to edit listing");
          navigate("/");
          return;
        }
    ...
    <main className=" max-w-md px-2 mx-auto">
      <form onSubmit={handleSubmit}>
        ...
        <button className=" bg-amber-900 w-full py-4 px-7 font-medium text-white rounded hover:bg-amber-950 hover:text-white focus:bg-amber-800 transition duration-300 ease-out mb-6">
          {isLoading ? <Loader /> : "Update Listing"}
        </button>
      </form>
    </main>
  );
